...
class LayerTransformer {
 public:
  LayerTransformer(const int device = -1) : device_(device) {}
  ...
  using TransformerDeserializer =
      std::unique_ptr<LayerTransformer> (*)(const syrenn_server::Layer&, const int device);
  ...
  virtual void TransformUPolytopeWithoutCompute(UPolytope *inout) const {
    throw "Unimplemented";
  }
  ...
  virtual void ComputeDevice(RMMatrixXfDevice *inout) const = 0;
  ...
  mutable uint64_t last_split_scale = 0;
  int device_ = -1;
};
