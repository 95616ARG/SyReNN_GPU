...
TEST(Conv2DTransformer, ComputeDevice) {
  const size_t points = 10,
               height = 4,
               width = 8,
               channels = 2,
  ...
  Conv2DTransformer(filters, biases, window_data, -1).Compute(&batch);
  Conv2DTransformer(filters, biases, window_data, 0).ComputeDevice(&batchDevice);

  EXPECT_EQ(batch.rows(), static_cast<int>(points));
  EXPECT_EQ(batchDevice.rows(), static_cast<int>(points));
