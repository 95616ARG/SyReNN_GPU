...
std::unique_ptr<LayerTransformer> ArgMaxTransformer::Deserialize(
  ...
  return std::unique_ptr<ArgMaxTransformer>(new ArgMaxTransformer(device));
}
...
std::vector<int> & ArgMaxTransformer::PointSignBatch(const RMMatrixXf &vertices,
  ...
  for (size_t vertex_idx = 0; vertex_idx < n_vertices; vertex_idx++) {
    PWLTransformer::point_sign[vertex_idx] = PointSign(vertices.row(vertex_idx),  plane_idx);
  }
