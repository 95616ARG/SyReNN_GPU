...
def RotatePlane(a, b, c):
    ...
    return M
class CIFAR10PlanesExperiment(Experiment):
    ...
    def get_points_cifar10(self, n_points = 3, seed=24):
        dataset = self.load_input_data("cifar10_test" ,False)
        ...
        network_name = "%s_relu_%d_%d" % ("cifar10", 4, 100)
    ...
    def run_for_network(self, network_type, input_plane, seed, data_csv, device, dry_run=False):
    ...
    def run(self):
        ...
        for i, (net, spec) in enumerate(networks_to_run):
            print(f"\t{net}_relu_{spec}_model")
        ...
        for seed in seeds:
            ...
            plane_labels = []
            for i in range(1):
                points, labels = self.get_points_cifar10(n_points=3, seed=seed+i)
                planes.append(points)
                plane_labels.append(labels)
            image_shape = (32, 32, 3)
            ...
            for device in devices:
                for seed in seeds:
                    self.run_for_network(
                        network_type,
                        planes_config[seed],
                        seed, data_csv, device, dry_run=dry_run)
    ...
    def analyze(self):
        ...
        for row in data_csv:
            ...
            cached_M_path = f"cifar10_seed_{seed}_M.npy"
            try:
                M = np.load(cached_M_path, allow_pickle=True)
                print(f"(post) Using cached `cifar10_seed_{seed}_M.npy`")
            except:
                ...
                print(f"(post) Computed and cached M `cifar10_seed_{seed}_M.npy`")
    ...
    def plot_polytopes(self, upolytope, preimages_transformed, highlights, image_shape, figure_label=""):
        ...
        plt.savefig(f"{figure_label}.png" , dpi=600)

if __name__ == "__main__":
    label = input("Label for this set of CIFAR-10 experiments (can be empty): ")
    if not (label == ""):
        label = "_" + label
    exp_name = f"cifar10_planes{label}"
    CIFAR10PlanesExperiment(exp_name).main()
