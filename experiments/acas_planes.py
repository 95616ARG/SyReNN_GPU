...
class ACASPlanesExperiment(ACASLinesExperiment):
    ...
    def run_for_network(self, network_name, data_csv, device):
        ...
        for scenario in scenarios:
            ...
            classifier = PlanesClassifier(network, [processed_plane],
                                          preimages=True)
            classifier.partial_compute(include_post=False, # Doesn't receive post-images from server.
                                       compute_pre=True,   # Compute pre-images on server.
                                       fuse_classify=True, # Fuse classification in one request.
                                       post_process=True,  # Process response to legacy structure.
                                       device = device)    # Designate the device that server will use.
            # classifier.partial_compute()
            fhat_size = len(classifier.transformed_planes[0])
            classified = classifier.compute()[0]
            timing = classifier.timing[0]
            # print(duration)
            # print(fhat_size, duration)
            # print("t_affine:", timing.affine)
            # print("t_pwl:", timing.pwl)
            ...
            # print("t_total:", timing.affine + timing.pwl + timing.preimage + timing.classify_argmax + timing.classify_extract)
            duration = timing.affine + timing.pwl + timing.preimage + timing.classify_argmax + timing.classify_extract
    ...
    def run(self):
        ...
        device = int(input("SyReNN Device ID (-1 for CPU): "))
        ...
        for i in range(1, 6):
            for j in range(1, 10):
                ...
                self.run_for_network(network_name, data_csv, device)
                models_run_for += 1
