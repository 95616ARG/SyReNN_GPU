...
class Network:
    ...
    def transform_planes(
        ...
        transformed = transformer_client.transform_planes(
            self,
            planes,
            ...
            device = device,
            dry_run = dry_run)
        print("(SyReNN) Server time:", (timer() - start) * 1000, "ms")
        ...
        if not post_process:
            for i, input_plane in enumerate(planes):
                transformed[i]["input_plane"] = input_plane
                transformed[i]["input_plane_idxes"] = [j for j, _ in enumerate(input_plane)]

        if (not compute_pre) and compute_preimages:
            start = timer()
            if post_process:
                for i, input_plane in enumerate(planes):
                    upolytope, timing = transformed[i]

                    for j, vpolytope in enumerate(upolytope):
                        if fuse_classify:
                            if include_post:
                                combinations, postimages, label = vpolytope
                                # (n_post, n_pre).(n_pre, n_dims)
                                preimages = np.matmul(combinations, input_plane)
                                upolytope[j] = (preimages, postimages, label)
                            else:
                                combinations, label = vpolytope
                                # (n_post, n_pre).(n_pre, n_dims)
                                preimages = np.matmul(combinations, input_plane)
                                upolytope[j] = (preimages, label)
                        else:
                            if include_post:
                                combinations, postimages = vpolytope
                                # (n_post, n_pre).(n_pre, n_dims)
                                preimages = np.matmul(combinations, input_plane)
                                upolytope[j] = (preimages, postimages)
                            else:
                                combinations = vpolytope
                                # (n_post, n_pre).(n_pre, n_dims)
                                preimages = np.matmul(combinations, input_plane)
                                upolytope[j] = preimages

                        transformed[i] = (upolytope, timing)
            else:
                for i, input_plane in enumerate(planes):
                    transformed[i]["pre-images"] = np.matmul(transformed[i]["combinations"], input_plane)

            print("compute preimage:", (timer() - start) * 1000, "ms")
        return transformed
